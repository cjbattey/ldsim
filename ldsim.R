...
plot_population <- function(pop,r){
  ...
  if(any(pd$recomb==1)){
    cplot <- ggplot(data=pd,aes(x=x,xend=xend,y=y,yend=yend))+
      ...
      geom_segment(data=subset(pd,recomb==1)[seq(1,nrow(pd),2),],aes(x=r/2-r*0.5,xend=r/2+r*0.5,y=y,yend=y+1),color="purple")+
      geom_segment(data=subset(pd,recomb==1)[seq(2,nrow(pd),2),],aes(x=r/2-r*0.5,xend=r/2+r*0.5,y=y,yend=y-1),color="purple")+
...
#testing drop in LD over time 
r <- 0.5
gen <- 100
pop <- data.frame(l1=c(rep("A",5),
                       rep("a",5)),
                  l2=c(rep("B",5),
                       rep("b",5)))
plot_population(pop,r)
pop <- makegametes(pop,r)
plot_population(pop,r)
...
dpred <- LDpred(d$D[1],r,gen)
d <- cbind(d,dpred)
names(d) <- c("gen","simulation","theory")
md <- melt(d,id.vars = "gen")

ggplot(data=d,aes(x=gen,y=value))+
  ...
  geom_line(data=md,aes(col=variable))+
  geom_point(data=subset(md,variable=="simulation"),color="black",size=1)
